...
###############
# remove first space and add a space at the end. This is because we want it in the beginning.
[ -z "$LP_ERR" ] || { # if not empty.
    LP_ERR="$(sed "s/^ //" <<< "${LP_ERR}")"
    LP_ERR="\e[91m✘\e[0m${LP_ERR}" # add red cross in the start of error code
    LP_ERR=" $LP_ERR"
}
...
LP_PS1="${LP_PS1}${LP_ERR}${LP_RUNTIME}${LP_MARK_PREFIX}${LP_MARK}${LP_PS1_POSTFIX}"
